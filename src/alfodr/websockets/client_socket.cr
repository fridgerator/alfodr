...
module Alfodr
  module WebSockets
    ...
    #
    # ```crystal
    # struct UserSocket < Alfodr::Websockets::ClientSocket
    ...
    abstract struct ClientSocket
      ...
      protected getter raw_params : Alfodr::Router::Params
      ...
      def initialize(@socket, @context)
        ...
        @params = Alfodr::Validators::Params.new(@raw_params)
        @socket.on_pong do
          @pongs.push(Time.now)
      ...
      protected def on_message(message)
        if @socket.closed?
          Alfodr.logger.error "Ignoring message sent to closed socket"
        else
