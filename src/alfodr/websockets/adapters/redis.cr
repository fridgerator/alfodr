...
module Alfodr::WebSockets::Adapters
  ...
  class RedisAdapter
    ...
    def initialize
      @subscriber = Redis.new(url: ENV["ALFODR_REDIS_URL"]? || "redis://localhost:6379")
      @publisher = Redis.new(url: ENV["ALFODR_REDIS_URL"]? || "redis://localhost:6379")
    end
